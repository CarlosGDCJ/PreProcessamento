...
CREATE FUNCTION preProcessa () RETURNS VOID AS $$
DECLARE 
    temp TEXT;
    rvmin TEXT;
    rvmax TEXT;
    ...
    curs refcursor;
BEGIN
    OPEN curs FOR SELECT DISTINCT t_rec FROM unifmin ORDER BY t_rec;
    LOOP
        ...
        EXECUTE format('
            ...
            FROM unifmin WHERE ''%s'' LIKE t_rec
            ', temp);
        ...
        DELETE FROM unifmin WHERE t_rec LIKE (SELECT t_rec FROM Intervalo LIMIT 1);
    END LOOP;
    CLOSE curs;
